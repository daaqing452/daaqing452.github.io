...
//				}catch(e){}
	...
	buildSelectWithPresetData : function($select,data) {
		...
		if($select.data("FormWidget").notFirst == null){
			if($select.attr("multiple")!="multiple" && $select.data("FormWidget").currentValue != "" && resultDataOption.indexOf("<option value ='"+ $select.data("FormWidget").currentValue +"'") == -1){//如果选项里没有当前选中的值，需要补充此选项
			...
			}else if($select.attr("multiple")=="multiple" && $select.data("FormWidget").currentValue != "" ){
		...
		if($select.data("FormWidget").notFirst == null){
			if($select.attr("multiple")!="multiple" && $select.data("FormWidget").currentValue != ""){
			...
			}else if($select.attr("multiple")!="multiple" && this.getCurrentState() == "填写状态" && $select.data("FormWidget").presetValue != null){
...
//		},
		...
		autocompleteFp : function(_this , config,param){
			$(_this||$("input[autocomplete=true]")||$("input[automatictp=true]")).each(function(i,o){
				$(o).autocomplete($.extend({
					items : $(o).attr("items") || 20 ,
			        source:function(query,process){
